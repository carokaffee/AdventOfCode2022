...
def parse_input(data):
    ...
    for chunk in data:
        ...
        operation[monkey] = eval("lambda old: " + lines[2].split(': ')[1].split(' = ')[1])
        modulo[monkey] = int(lines[3].split()[-1])
        action[monkey] = {True: int(lines[4].split()[-1]), False: int(lines[5].split()[-1])}
    ...
    for monkey in monkeys:
        ...
        for item_mod_monkeys in items[monkey]:
            item_mod_monkeys.update({
                m: operation[monkey](item) % modulo[m] for m, item in item_mod_monkeys.items()
            })
